...
for (let car in inventory) carModels.push(inventory[car].car_model);

carModels = carModels.sort().join(', ');
...
for (let car in inventory) carYears.push(inventory[car].car_year);
...
for (let year in carYears) {
  let y = carYears[year];

  if (y < 2000) oldCars.push(y);
}

console.log(`There ${oldCars.length > 1 || oldCars.length == 0 ? "are" : "is"} ${oldCars.length} ${oldCars.length > 1 || oldCars.length == 0 ? "cars" : "car"} made before 2000 in stock.`);
...
for (let car in inventory) {
  ...
  if (c.car_make == "BMW" || c.car_make == "Audi") BMWAndAudi.push(c);
}
